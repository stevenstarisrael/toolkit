  ...
  return (
    <div className="w-full max-w-md mx-auto px-6 py-10">
      <div className="relative group bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 shadow-2xl">
        <h2 className="text-2xl font-bold text-purple-200 mb-4">Compound Interest Calculator</h2>
        <div className="flex flex-col gap-4 mb-4">
          ...
          <select className="p-3 rounded-lg bg-transparent text-white border border-purple-400 outline-none focus:ring-2 focus:ring-purple-400" value={frequency} onChange={e => setFrequency(Number(e.target.value))}>
            {FREQUENCIES.map(f => (
        ...
        {result !== null && (
          <div className="mt-4 text-lg text-purple-100">
            Compound Amount: <span className="font-semibold text-white">₹{result.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
          </div>
        )}
      </div>
    </div>
