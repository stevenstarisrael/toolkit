...
export default function TextToBinary() {
  ...
  const convert = () => {
    setOutput(direction === 'text2bin' ? textToBinary(input) : binaryToText(input));
  };
    ...
    <div className="w-full max-w-2xl mx-auto px-6 py-10">
      <div className="relative group">
        ...
        <div className="relative bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 shadow-2xl">
          <h2 className="text-2xl font-bold text-purple-200 mb-4">Text &lt;-&gt; Binary</h2>
          <textarea
            ...
            placeholder={direction === 'text2bin' ? 'Enter text...' : 'Enter binary (8-bit, space separated)...'}
          />
          <div className="flex flex-wrap gap-2 mb-2">
            <label className="flex items-center gap-2 text-purple-200 text-sm select-none cursor-pointer">
              <input type="radio" name="direction" value="text2bin" checked={direction === 'text2bin'} onChange={() => setDirection('text2bin')} className="accent-purple-500 w-4 h-4" />
              Text to Binary
            </label>
            <label className="flex items-center gap-2 text-purple-200 text-sm select-none cursor-pointer">
              <input type="radio" name="direction" value="bin2text" checked={direction === 'bin2text'} onChange={() => setDirection('bin2text')} className="accent-purple-500 w-4 h-4" />
              Binary to Text
            </label>
            <button
